...
const LeaseAgreementScreen = ({ route, navigation }) => {
  ...
  const role = "Landlord"; // user being rated is landlord here
  
   // To store the fetched lease data
  ...
  useEffect(() => {
    const fetchLeaseAgreement = async () => {
      try {
        ...
        setRatedEntityId(response.data.landlordId._id);
        setReviewerId(response.data.tenantId._id);
  ...
  const handleRatingSubmit = async (ratingData) => {
    ...
    try {
      ...
      Alert.alert('Success', response.data.message); // Notify user of success
